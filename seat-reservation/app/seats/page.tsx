...
export default function SeatsPage() {
  ...
  const getDefaultTimes = () => {
    const now = new Date();
    const startObj = new Date(now.getTime() + 15 * 60000);
    const endObj = new Date(startObj.getTime() + 2 * 60 * 60 * 1000);
    const getLocalISO = (d: Date) => new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    return { start: getLocalISO(startObj), end: getLocalISO(endObj) };
  };
...
useEffect(() => {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/me/notifications`, {
        credentials: "include"
      });
  ...
  const fetchSeats = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/seats/available?start_time=${startTime}:00&end_time=${endTime}:00`,
      );
      if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white p-8">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            {userRole === "admin" && (
              <Link
                href="/admin"
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg border border-emerald-500 shadow-lg transition-colors font-bold"
              >
                Admin Panel
              </Link>
            )}
            <Link
              href="/dashboard"
              className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white px-4 py-2 rounded-lg border border-zinc-700 transition-colors"
            >
            <User size={18} />
            My Reservations
          </Link>
       ...
       <div className="mb-8 p-5 bg-zinc-800 rounded-lg border border-zinc-700 flex flex-col gap-5 shadow-md">
          <div className="flex flex-col border-b border-zinc-700 pb-5">
            <label className="text-zinc-300 font-bold mb-2">Select Office:</label>
            <select 
              value={selectedOffice} 
              onChange={(e) => setSelectedOffice(e.target.value)}
              className="bg-zinc-900 border border-zinc-600 rounded-md p-3 text-white text-lg font-semibold focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <input
                ...
                className="bg-zinc-900 border border-zinc-700 rounded-md p-2 text-white text-sm focus:outline-none focus:border-emerald-500"
              />
            </div>
          ...
          <div className="flex flex-col border-t border-zinc-700 pt-5 mt-2">
            <label className="text-zinc-300 font-bold mb-2">Repeat Reservation:</label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <select 
                value={recurrenceRule} 
                onChange={(e) => setRecurrenceRule(e.target.value)}
                className="bg-zinc-900 border border-zinc-700 rounded-md p-2 text-white text-sm focus:outline-none focus:border-emerald-500 cursor-pointer"
              >
              ...
              {recurrenceRule !== "none" && (
                <div className="flex flex-col">
                  <label className="text-xs text-zinc-400 mb-1">Until (End Date):</label>
                  <input
                    type="date"
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-8 mb-10">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xl font-bold text-zinc-300">Select your desk</h2>
              <div className="flex bg-zinc-800 rounded-lg p-1 border border-zinc-700">
                <button
                  onClick={() => setViewMode("grid")}
                  className={`flex items-center gap-2 px-4 py-2 rounded-md font-bold text-sm transition-colors ${viewMode === "grid" ? "bg-emerald-600 text-white shadow" : "text-zinc-400 hover:text-zinc-200"}`}
                >
                  <Grid size={16} /> Grid
                </button>
                <button
                  onClick={() => setViewMode("map")}
                  className={`flex items-center gap-2 px-4 py-2 rounded-md font-bold text-sm transition-colors ${viewMode === "map" ? "bg-emerald-600 text-white shadow" : "text-zinc-400 hover:text-zinc-200"}`}
                >
                  <Map size={16} /> Map
                </button>
              </div>
            ...
            {viewMode === "map" ? (
              ...
              >
                <div className="flex items-center justify-between mb-8 border-b border-zinc-700 pb-4">
                  ...
                  <span className="text-zinc-400 text-sm font-medium bg-zinc-900 px-3 py-1 rounded-full border border-zinc-700">
                ...
                <div
                  className={`grid gap-4 ${
                    zone === "Open Space"
                      ...
                      : zone === "Quiet Zone"
                        ? "grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-12 place-items-center bg-zinc-900/50 p-6 rounded-xl border border-zinc-800"
                      ...
                      <button
                        ...
                        className={`relative group w-24 h-24 flex flex-col items-center justify-center rounded-xl font-bold transition-all duration-300 shadow-md ${
                          !seat.is_active
                            ...
                            : !seat.is_available
                              ? "bg-zinc-800 text-zinc-600 cursor-not-allowed border-2 border-zinc-900 opacity-60"
                              : selectedSeat === seat.id
                                ? "bg-emerald-600 text-white scale-110 ring-4 ring-emerald-400 ring-opacity-50 z-10"
                                : "bg-gradient-to-b from-zinc-700 to-zinc-800 hover:from-zinc-600 hover:to-zinc-700 text-zinc-200 border-2 border-zinc-600 hover:border-emerald-400 hover:shadow-emerald-900/50 hover:shadow-lg cursor-pointer"
                        }`}
                      >
                        ...
                        {seat.has_monitor && seat.is_active && (
                          <div
                            className="absolute -top-3 -right-3 bg-zinc-800 text-emerald-400 w-8 h-8 flex items-center justify-center rounded-xl border border-zinc-700 shadow-lg rotate-12 group-hover:rotate-0 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300"
