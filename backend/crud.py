...
def get_available_seats(db: Session, start_time: datetime, end_time: datetime):
    ...
    return [
        schemas.SeatResponse(id=seat.id, seat_number=seat.seat_number, is_available=seat.id not in reserved_ids,office_name=seat.office_name, zone=seat.zone, desk_type=seat.desk_type, has_monitor=seat.has_monitor) #type: ignore
